...
名称: <input type="text" value="${name}" name="name"/>
	...
	<select onchange="changePageNo()" name="isShow">
		<option selected="selected" <c:if test="${isShow == 1}"> selected = "selected"></c:if>上架</option>
		<option selected="selected" <c:if test="${isShow == 0}"> selected = "selected"></c:if>下架</option>
	</select>
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<c:forEach items="${paginaation.list}" var="entry"></c:forEach>
...
<div class="page pb15">
	<span class="r inb_a page_b">
		<c:forEach items="${paginaation.pageView}" var="page">
			${page}
		</c:forEach>
